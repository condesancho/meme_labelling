...
MODEL_PATH = "../models/efficientnet_feat_ex/"
BATCH_SIZE = 64
...
ft_model = load_model(MODEL_PATH + "efficientnet_fine_tuned.h5")
...
for idx, image_path in enumerate(candidate_images):
    ...
    img = get_image(file_path, input_shape, architecture="efficientnet")
    if img is not None:
        if idx % 100 == 0:
            print(
